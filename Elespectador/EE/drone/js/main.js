...
/* carga menus seleccion del formulario */
$( document ).ready(function() {
$("#div_ciudad").load( "php/response.php", { action: 'combos()', cmbid: 'ciudad', llve: 'distinct(ciudad) as cty', tabla: 'drone_puntos', where: '1', order: 'ciudad', fisrt: 'Elija ciudad de redenci&oacute;n' }, function(response, status, xhr) {
$('#combo_ciudad').change(function() {
document.cargando="full";$(".nivo-lightbox-close").hide();$("#cargar").trigger( "click" );
$("#div_puntos").load( "php/response.php",{ action: 'combos()', cmbid: 'puntos', llve: 'almacen as alm', tabla: 'drone_puntos', where: 'ciudad like #1'+$(this).val()+'#2', order: 'almacen', primary: 'cod_almacen,',fisrt: 'Seleccione el punto de redenci&oacute;n' },
function(response, status, xhr) {
 if(status=="success"){
 $(".nivo-lightbox-close").trigger( "click" );document.cargando="off";$(".nivo-lightbox-overlay").hide();
$('#combo_puntos').change(function() {
document.cargando="full";$(".nivo-lightbox-close").hide();$("#cargar").trigger( "click" );$(".nivo-lightbox-overlay").show();
$("#direscon").load( "php/response.php", { action: 'lddir()', idalm: $(this).val()}, function(response, estado, xhr) {
$(this).show();$("#direccion").focus();$("#almacen_code").val($("#combo_puntos").val());
if(estado=="success"){
$(".nivo-lightbox-close").trigger( "click" );document.cargando="off";$(".nivo-lightbox-overlay").hide();
}
});});}});});});
