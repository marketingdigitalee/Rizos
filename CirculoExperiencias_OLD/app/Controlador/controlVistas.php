...
/*----------------------FUNCIONES PAGINA PRINCIPAL  ------------*/
	function cargar_vistaHome(){
		$pagina = $this->load_page('app/Vista/page.php');
		$top = $this->load_page('app/Vista/desing1/header.php');
		$nav = $this->load_page('app/Vista/vacio.php');
		$content = $this->load_page('app/Vista/desing1/slider.php');
		$bottom = $this->load_page('app/Vista/desing2/menu.php');
		
		$pagina = $this->replace_content('/\#TOP\#/ms' ,$top , $pagina);
		$pagina = $this->replace_content('/\#NAV\#/ms' ,'' , $pagina);
		$pagina = $this->replace_content('/\#CONTENIDO\#/ms' ,$content , $pagina);
		$pagina = $this->replace_content('/\#BOTTOM\#/ms' ,$bottom , $pagina);
...
/*----------------------FUNCIONES PAGINA SECUNDARIAS  ------------*/

	function cargar_secundaria(){

		$pagina = $this->load_page('app/Vista/page.php');
		$top = $this->load_page('app/Vista/desing1/header.php');
		$nav = $this->load_page('app/Vista/vacio.php');
		$bottom = $this->load_page('app/Vista/desing1/menu.php');
...
/*--------------- FUNCION DE LLAMADO -----------*/
	...
	function cargarPag($pagina){
		if(!is_null($pagina)){
			$ppal = $this->cargar_secundaria();
			$url = 'app/Vista/dising2/'.$pagina.'.php';
			$cargar = $this->load_page($url);
			$ppal = $this->replace_content('/\#CONTENIDO\#/ms' ,$cargar , $ppal);
		}else{
			$ppal = $this->cargar_vistaHome();
		}
